...
SharkMiss(int i)
{
    ...
    for (j = 0; j < NUM_SHARKS; j++) {
        if (j != i) {
            X = progState.sharks[j].x - progState.sharks[i].x;
            Y = progState.sharks[j].y - progState.sharks[i].y;
            Z = progState.sharks[j].z - progState.sharks[i].z;

            R = sqrt(X * X + Y * Y + Z * Z);

            avoid = 1.0;
            thetal = progState.sharks[i].theta;

            if (R < SHARKSIZE) {
                if (Z > 0.0) {
                    progState.sharks[i].theta -= avoid;
                } else {
                    progState.sharks[i].theta += avoid;
                }
            }
            progState.sharks[i].dtheta += (progState.sharks[i].theta - thetal);
        }
    }
}
