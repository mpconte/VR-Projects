...
/* The Mesa GLU on Linux does not define GLUquadric */
...
void drawperfobj(perfobj_t *perfobj)
{
    ...
    for (;;) {
		...
		case PD_PAPER_PLANE_POS:        /* contains the pushmatrix */
			...
			glRotatef(*(vdata_ptr + 5), 1.0, 0.0, 0.0);
			{
			  ...
			  if (dm > 1.0)
			    dm = 1.0;
			  for(k = 0; k < 4; k++)
			    cl[k] = dm*bad[k] + (1.0-dm)*ambient[k];
			  glColor4fv(cl);
			}
			flagsptr += 1;
			break;
		...
		case PD_TEXTURE_BIND:
	        ...
	        case PD_DRAW_SHOT:
		    glPushMatrix();
		    glTranslatef(*(vdata_ptr + 1),*(vdata_ptr + 2),*(vdata_ptr + 3));
		    {
			...
			gluQuadricDrawStyle(q,GLU_FILL);
		      ...
		      glDisable(GL_LIGHTING);
		      glEnable(GL_BLEND);
		      glColor4f(0.2,0.0,0.0,vdata_ptr[5]);
		      gluSphere(q,*(vdata_ptr + 4),4,4);
		      glDisable(GL_BLEND);
		      glEnable(GL_LIGHTING);
		    }
		    glPopMatrix();
