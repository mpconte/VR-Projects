...
/** function veClockGetRef
    ...
    @returns
    A string describing the real-world time at clock time <i>ref</i>.
**/
char *veClockGetRef(long ref); 

/** function veClockMakeRef
    Converts a given clock value into a reference string.  Typically
    only used by <code>veClockGetRef()</code>.  This function is
    platform-specific and may not be thread-safe.

    @param hr
    The absolute clock value for which to generate a reference string.
...
/** function veClockParseRef
    Parses a clock-reference string relative to a time point and
