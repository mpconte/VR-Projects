...
typedef Bool               (*ISD_EXT_DATA_FN)       ( ISD_TRACKER_HANDLE, ISD_EXTENDED_DATA_TYPE * );
...
ISD_EXT_DATA_FN       _ISD_GetExtendedData          = NULL;   
...
static DLL *ISD_LoadLib( void )
{
    if(hLib = dll_load( ISD_LIB_NAME ))
    {
        ...
        _ISD_GetExtendedData        = ( ISD_EXT_DATA_FN )       dll_entrypoint( hLib, "ISD_GetExtendedData" );
...
/******************************************************************************/
Bool ISD_GetExtendedData( 
                         ISD_TRACKER_HANDLE handle, 
                         ISD_EXTENDED_DATA_TYPE *Data 
                         )
{
    if( !_ISD_GetExtendedData ) return FALSE;
    return (*_ISD_GetExtendedData)( handle, Data );
}
