...
/* modified slightly for use with VE */
  ...
  int xoff, int yoff, int advance, int data)
{
  if (width == 0 || height == 0) {
    ...
    printf("static GLubyte ch%ddata[] = { 0x0 };\n", num);
    printf("static VeiGlBitmapChar ch%d = {", num);
    printf("%d,", 0);
    printf("%d,", 0);
    printf("%d,", xoff);
    printf("%d,", yoff);
...
captureXFont(Display * dpy, Font font, char *xfont, char *name)
{
  ...
  printf("static VeiGlBitmapChar * chars[] = {\n");
  ...
  printf("};\n\n");
  printf("VeiGlBitmapFont %s_bitmap = {\n", name);
  printf("\"%s\",\n", xfont);
  printf("%d,\n", last - first + 1);
  printf("%d,\n", first);
  printf("%d,\n", height);
