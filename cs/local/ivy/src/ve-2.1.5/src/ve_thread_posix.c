...
int veThreadInit(VeThread *t, void *(*func)(void *), void *arg, int priority,
  ...
  if (t)
    *t = p;
  VE_DEBUG(10,("[%x] thread created",(int)p));
  return 0;
}
...
void veThrMutexLock(VeThrMutex *m) {
  VE_DEBUG(10,("[%d] requesting lock on mutex 0x%x", 
	      (int) pthread_self(), (unsigned int)m));
  veThrMutexQuietLock(m);
  VE_DEBUG(10,("[%d] acquired lock on mutex 0x%x", 
	       (int) pthread_self(), (unsigned int)m));
}
...
void veThrMutexUnlock(VeThrMutex *m) {
  VE_DEBUG(10,("[%d] releasing lock on mutex 0x%x",
	       (int) pthread_self(), (unsigned int)m));
...
VeThrCond *veThrCondCreate() {
  pthread_cond_t *c;
  c = fmalloc(sizeof(pthread_cond_t));
...
void veThrCondWait(VeThrCond *c, VeThrMutex *m) {
  VE_DEBUG(10,("[%d] waiting on cond 0x%x mutex 0x%x",
  ...
  VE_DEBUG(10,("[%d] woken up from cond 0x%x mutex 0x%x",
	      (int) pthread_self(), (unsigned int) c, (unsigned int)m));
}
...
void veThrCondTimedWait(VeThrCond *c, VeThrMutex *m, long ms) {
  ...
  VE_DEBUG(10,("[%d] timed wait on cond 0x%x mutex 0x%x",
	       (int) pthread_self(), (unsigned int) c, (unsigned int)m));
...
void veThrCondSignal(VeThrCond *c) {
  VE_DEBUG(10,("[%d] broadcasting on cond 0x%x",
