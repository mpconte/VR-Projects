...
void veLockCallbacks() {
  VE_DEBUGM(4,("[%2x] Locking callbacks",veThreadId()));
  veThrMutexLock(cback_mutex);
  VE_DEBUGM(4,("[%2x] Callbacks locked",veThreadId()));
  cbacks_locked = 1;
}

void veUnlockCallbacks() {
  cbacks_locked = 0;
  VE_DEBUGM(4,("[%2x] Unlocking callbacks",veThreadId()));
...
void veGetWindowView(VeWindow *w, VeFrame *default_eye,
  ...
  veGetWindowClipView(w,default_eye,eye,core_znear,core_zfar,wv);
}
...
void veLockFrame(void) {
  ...
  VE_DEBUGM(5,("[%d] veLockFrame enter",veThreadId()));
  while (fl_epending)
    veThrCondWait(fl_eclear,fl_mutex);
  while (fl_locked < 0)
    veThrCondWait(fl_zero,fl_mutex);
  fl_locked++;
  VE_DEBUGM(5,("[%d] veLockFrame exit",veThreadId()));
  veThrMutexUnlock(fl_mutex);
}

void veUnlockFrame(void) {
  veThrMutexLock(fl_mutex);
  VE_DEBUGM(5,("[%d] veUnlockFrame enter",veThreadId()));
  assert(fl_locked > 0);
  fl_locked--;
  if (fl_locked == 0)
    veThrCondBcast(fl_zero);
  VE_DEBUGM(5,("[%d] veUnlockFrame exit",veThreadId()));
  veThrMutexUnlock(fl_mutex);
}

void veLockFrameExcl(void) {
  veThrMutexLock(fl_mutex);
  VE_DEBUGM(5,("[%d] veLockFrameExcl enter",veThreadId()));
  if (fl_locked != 0) {
    fl_epending++;
    while(fl_locked != 0)
      veThrCondWait(fl_zero,fl_mutex);
  ...
  fl_locked = -1;
  VE_DEBUGM(5,("[%d] veLockFrameExcl exit",veThreadId()));
  veThrMutexUnlock(fl_mutex);
}
...
void veUnlockFrameExcl(void) {
  ...
  fl_locked = 0;
  veThrCondBcast(fl_zero);
  VE_DEBUGM(5,("[%d] veUnlockFrameExcl exit",veThreadId()));
  veThrMutexUnlock(fl_mutex);
}
