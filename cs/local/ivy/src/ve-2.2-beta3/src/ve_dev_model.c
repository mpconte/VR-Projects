...
VeDeviceEContent *veDeviceEContentCreate(VeDeviceEType type, int vsize) {
  ...
  case VE_ELEM_TRIGGER:
    c = (VeDeviceEContent *)veAllocObj(VeDeviceE_Trigger);
    break;
  case VE_ELEM_SWITCH:
    c = (VeDeviceEContent *)veAllocObj(VeDeviceE_Switch);
    break;
  case VE_ELEM_VALUATOR:
    c = (VeDeviceEContent *)veAllocObj(VeDeviceE_Valuator);
    break;
  case VE_ELEM_VECTOR:
    c = (VeDeviceEContent *)veAllocObj(VeDeviceE_Vector);
    {
      VeDeviceE_Vector *vec = (VeDeviceE_Vector *)c;
      vec->size = vsize;
      if (vsize > 0) {
	vec->min = veAlloc(vsize*sizeof(float),1);
	vec->max = veAlloc(vsize*sizeof(float),1);
	vec->value = veAlloc(vsize*sizeof(float),1);
      }
    }
    break;
  case VE_ELEM_KEYBOARD:
    c = (VeDeviceEContent *)veAllocObj(VeDeviceE_Keyboard);
    break;
  default:
