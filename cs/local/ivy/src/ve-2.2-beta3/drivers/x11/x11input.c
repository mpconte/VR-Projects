...
static void *kbd_thread(void *v) {
  ...
  VE_DEBUGM(1,("x11keyboard %s: thread started", d->name));
  ...
  VE_DEBUGM(1,("x11keyboard %s: all windows snooped", d->name));

  ke = (XKeyEvent *)&e;
  while(1) {
    XNextEvent(dpy,&e);
    if (e.type != KeyPress && e.type != KeyRelease) {
      VE_DEBUGM(1,("x11keyboard %s: discarding event", d->name));
      continue;
    }
    /* build event */
    veLockFrame();
    ...
    VE_DEBUGM(2,("x11keyboard %s: keycode %d",d->name,k->key));
    if (xstr = veKeysymToString(k->key)) {
      VE_DEBUGM(2,("x11keyboard %s: using name '%s'",d->name,xstr));
      ve->elem = veDupString(xstr);
    } else {
      VE_DEBUGM(2,("x11keyboard %s: using ascii name '%s'",d->name,str));
      ve->elem = veDupString(str);
    }
    ve->timestamp = ke->time; /* this is horribly, horribly wrong */
    k->state = (e.type == KeyPress) ? 1 : 0;
    VE_DEBUGM(1,("x11keyboard %s: event %d '%s'",d->name,k->key,ve->elem));
...
static void *mouse_thread(void *v) {
  ...
  VE_DEBUGM(1,("x11mouse %s: thread started", d->name));
  ...
  VE_DEBUGM(1,("x11mouse %s: events registered - entering loop", d->name));
  ...
  while(1) {
    ...
    case ButtonRelease:
      ...
      case Button4: enm = "wheel_up"; break;
      case Button5: enm = "wheel_down"; break;
      default: enm = NULL;
      }
      if (!enm) {
	VE_DEBUGM(2,("x11mouse %s: ignoring unknown button event",d->name));
	continue;
      }
      veLockFrame();
	 ...
	 absolute wall clock, this is the best we can do for now. */
      ...
      /* process this event */
      VE_DEBUGM(2,("x11mouse %s: button %s %d", ve->elem, sw->state));
      veDeviceInsertEvent(ve);
      veUnlockFrame();
      break;
	...
	if (next.type == MotionNotify) {
	  VE_DEBUGM(2,("x11mouse %s: skipping redundant motion event", d->name));
	  XNextEvent(dpy,&next);
	} else
	  break;
      }
      ...
      /* process this event */
      VE_DEBUGM(2,("x11mouse %s: motion vector %g %g", d->name,
...
static VeDevice *new_kbd_driver(VeDeviceDriver *driver, VeDeviceDesc *desc,
  ...
  VE_DEBUGM(2,("making new X11 keyboard device"));
  ...
  VE_DEBUGM(2,("returning new keyboard device: 0x%x",(unsigned)d));
  return d;
}
...
static VeDevice *new_mouse_driver(VeDeviceDriver *driver, VeDeviceDesc *desc,
  ...
  VE_DEBUGM(1,("x11mouse - creating new mouse driver"));
...
static VeDeviceDriver x11_kbd_drv = {
  "x11keyboard", new_kbd_driver
};

static VeDeviceDriver kbd_drv = {
  "keyboard", new_kbd_driver
};

static VeDeviceDriver x11_mouse_drv = {
  "x11mouse", new_mouse_driver
};

static VeDeviceDriver mouse_drv = {
  "mouse", new_mouse_driver
};

void VE_DRIVER_INIT(x11input) (void) {
  veDeviceAddDriver(&kbd_drv);
  veDeviceAddDriver(&mouse_drv);
  veDeviceAddDriver(&x11_kbd_drv);
  veDeviceAddDriver(&x11_mouse_drv);
}

void VE_DRIVER_PROBE(x11input) (void *phdl) {
  veDriverProvide(phdl,"input","x11keyboard");
  veDriverProvide(phdl,"input","x11mouse");
  veDriverProvide(phdl,"input","keyboard");
  veDriverProvide(phdl,"input","mouse");
}
