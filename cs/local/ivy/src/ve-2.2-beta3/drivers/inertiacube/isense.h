...
typedef struct
{
    ...
    float   AngVel[3];       // rad/sec, in sensor body coordinate frame. 
                             // This is the processed angular rate, with current biases 
                             // removed. This is the angular rate used to produce 
                             // orientation updates.

    float   VelNav[3];       // meter/sec, IS-900 only.  

    float   AccelNav[3];     // meter^2/sec, in the navigation (earth) coordinate frame. 
                             // This is the accelerometer measurements with calibration,  
                             // and current sensor orientation applied, and gravity  
                             // subtracted. Since accelerometer biases are not tracked, 
                             // only factory calibration can be applied, some 
                             // residual errors due to temperature variations remain.
    
    float   AccelBody[3];    // meter^2/sec, in sensor body coordinate frame. This is 
    ...
    float   TimeStamp;       // Seconds, reported only if requested.
    float   StillTime;       // IC2, IC3 and PCTracker only.

    Bool    ButtonState[ISD_MAX_BUTTONS];    // Only if requested 

    // Current hardware is limited to 10 channels, only 2 are used. 
    // The only device using this is the IS-900 wand that has a built-in
    // analog joystick. Channel 1 is x-axis rotation, channel 2 is y-axis rotation.
    ...
    float   GyroRate[3];     // Raw gyro output, only factory calibration is applied. 
                             // Some errors, due to temperature dependant gyro bias 
                             // drift, will remain.

    float   MagYaw;          // Magnetometer heading, computed based on current orientation.
    float   MagRaw[3];       // Raw magnetometer readings that are used to calculate MagYaw.

    BYTE    TrackingStatus;  // Tracking status byte 
    BYTE    CommIntegrity;   // Communication integrity of wireless link 
    BYTE    bReserved3;          
    BYTE    bReserved4;      // pack to 4 byte boundary

    float   fReserved2;
    float   fReserved3;
    float   fReserved4;
}
ISD_STATION_EXTENDED_DATA_TYPE;
...
typedef struct
{
    ISD_STATION_EXTENDED_DATA_TYPE Station[ISD_MAX_STATIONS];

} ISD_EXTENDED_DATA_TYPE;
...
DLLEXPORT Bool DLLENTRY ISD_GetExtendedData( 
                                            ISD_TRACKER_HANDLE handle, 
